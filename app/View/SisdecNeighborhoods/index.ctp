<div class="sisdecNeighborhoods index">
	<h2><?php echo __('Sisdec Neighborhoods'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id'); ?></th>
			<th><?php echo $this->Paginator->sort('nome_neighborhood'); ?></th>
			<th><?php echo $this->Paginator->sort('sisdec_place_id'); ?></th>
			<th><?php echo $this->Paginator->sort('created'); ?></th>
			<th><?php echo $this->Paginator->sort('modified'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	<?php foreach ($sisdecNeighborhoods as $sisdecNeighborhood): ?>
	<tr>
		<td><?php echo h($sisdecNeighborhood['SisdecNeighborhood']['id']); ?>&nbsp;</td>
		<td><?php echo h($sisdecNeighborhood['SisdecNeighborhood']['nome_neighborhood']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($sisdecNeighborhood['SisdecPlace']['id'], array('controller' => 'sisdec_places', 'action' => 'view', $sisdecNeighborhood['SisdecPlace']['id'])); ?>
		</td>
		<td><?php echo h($sisdecNeighborhood['SisdecNeighborhood']['created']); ?>&nbsp;</td>
		<td><?php echo h($sisdecNeighborhood['SisdecNeighborhood']['modified']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $sisdecNeighborhood['SisdecNeighborhood']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $sisdecNeighborhood['SisdecNeighborhood']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $sisdecNeighborhood['SisdecNeighborhood']['id']), null, __('Are you sure you want to delete # %s?', $sisdecNeighborhood['SisdecNeighborhood']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>
	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Sisdec Neighborhood'), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Sisdec Places'), array('controller' => 'sisdec_places', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Sisdec Place'), array('controller' => 'sisdec_places', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Sisdec Occurrences'), array('controller' => 'sisdec_occurrences', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Sisdec Occurrence'), array('controller' => 'sisdec_occurrences', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Sisdec Requesters'), array('controller' => 'sisdec_requesters', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Sisdec Requester'), array('controller' => 'sisdec_requesters', 'action' => 'add')); ?> </li>
	</ul>
</div>
